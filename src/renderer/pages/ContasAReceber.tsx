...
import { Search, CheckCircle2, SplitSquareHorizontal, Undo2, HandCoins } from 'lucide-react'
...
const SITUACAO_LABEL: Record<string, string> = {
  a_vencer: 'A receber', vencido: 'Vencido', pago: 'Recebido',
}
const SITUACAO_COLOR: Record<string, 'yellow' | 'red' | 'green'> = {
  a_vencer: 'yellow', vencido: 'red', pago: 'green',
}

// NOVO: mesma lógica de Contas a Pagar, mas para receitas — dar baixa
// (marcar como recebido) ou receber parcialmente, gerando uma nova
// parcela com o restante.
export default function ContasAReceber() {
  const empresaId  = useEmpresaStore(s => s.empresaId)
  const { format } = useCurrency()
  const { confirm, dialogProps } = useConfirm()

  const [itens, setItens]         = useState<ContaAReceber[]>([])
  const [loading, setLoading]     = useState(true)
  const [busca, setBusca]         = useState('')
  const [situacao, setSituacao]   = useState('')
  const [parcial, setParcial]     = useState<ContaAReceber | null>(null)
  ...
  const carregar = useCallback(() => {
    if (!empresaId) return
    setLoading(true)
    window.api.contasAReceber.listar({
  ...
  async function handleDarBaixa(c: ContaAReceber) {
    const ok = await confirm({
      title:   'Marcar como recebido',
      message: `Marcar "${c.descricao}" (${format(c.valor)}) como recebido?`,
    })
    if (!ok) return
    try {
      await window.api.contasAReceber.darBaixa({ id: c.id })
  ...
  async function handleReabrir(c: ContaAReceber) {
    const ok = await confirm({
      title:   'Reabrir conta',
      message: `Desfazer o recebimento de "${c.descricao}"? Ela volta a ficar pendente.`,
    })
    if (!ok) return
    try {
      await window.api.contasAReceber.reabrir(c.id)
  ...
  const totalAReceber = itens.filter(i => i.situacao === 'a_vencer').reduce((s, i) => s + i.valor, 0)
  const totalVencido  = itens.filter(i => i.situacao === 'vencido').reduce((s, i) => s + i.valor, 0)

  return (
    <div>
      <PageHeader title="Contas a Receber" subtitle="Receitas lançadas, por situação de recebimento" />

      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
          <p className="text-xs text-gray-300 mb-1">A receber</p>
          <p className="text-xl font-bold text-amber-400">{format(totalAReceber)}</p>
        </div>
      ...
      <div className="flex flex-wrap items-end gap-3 mb-4">
        ...
        <Select
          ...
          options={[
            ...
            { value: 'a_vencer', label: 'A receber' },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border">
                {['Descrição', 'Cliente/Fornecedor', 'Vencimento', 'Valor', 'Situação', ''].map(h => (
      ...
      {parcial && (
        <PagamentoParcialModal
          ...
          onConfirmar={p => window.api.contasAReceber.pagarParcial(p)}
          titulo="Recebimento parcial"
          labelValor="Valor recebido agora (R$)"
          labelBotao="Confirmar recebimento parcial"
        />
      )}
