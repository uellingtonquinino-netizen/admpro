...
export async function uploadDocumento(local:string, remote:string) { const {error}=await getSupabase().storage.from('documentos-rh').upload(remote,await readFile(local),{upsert:true}); if(error) throw new Error(error.message); return prefix+remote }
