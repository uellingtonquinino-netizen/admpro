...
type SerieSupervisor = { mes: string; admissoes: number; desligamentos: number; despesas: number }
...
function GraficosSupervisor({ resumo, serie }: { resumo: ResumoSupervisor; serie: SerieSupervisor[] }) {
  const totalAdmissoes = serie.reduce((total, item) => total + item.admissoes, 0)
  const totalDesligamentos = serie.reduce((total, item) => total + item.desligamentos, 0)
  const totalDespesas = serie.reduce((total, item) => total + item.despesas, 0)
  const turnover = resumo.colaboradores ? Math.round(((totalAdmissoes + totalDesligamentos) / resumo.colaboradores) * 100) : 0
  const distribuicao = [
    { name: 'Ativos', value: resumo.colaboradores, color: '#22c55e' },
    { name: 'Movimentações', value: totalAdmissoes + totalDesligamentos, color: '#3b82f6' },
    { name: 'Pendências', value: resumo.pendencias, color: '#f59e0b' },
  ].filter(item => item.value > 0)
  return <section className="mx-auto max-w-7xl pb-7">
    <p className="mb-3 text-xs font-bold uppercase tracking-widest text-brand-300">Visão geral — todas as obras</p>
    <div className="grid gap-4 xl:grid-cols-3">
      <section className="rounded-2xl border border-surface-border bg-surface p-5 shadow-panel"><div className="flex items-center justify-between"><h3 className="text-sm font-bold">Admissões e desligamentos</h3><span className="rounded-md border border-surface-border bg-surface-card px-2 py-1 text-[11px] text-gray-300">Últimos 6 meses</span></div><div className="mt-4 h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={serie} barGap={5}><CartesianGrid vertical={false} stroke="#2e405d" strokeDasharray="3 4" /><XAxis dataKey="mes" tick={{ fill: '#8fa5c5', fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis hide /><Tooltip cursor={{ fill: '#22314a' }} contentStyle={{ background: '#1d2b43', border: '1px solid #3b5175', borderRadius: 10 }} /><Bar dataKey="admissoes" name="Admissões" fill="#22c55e" radius={[5, 5, 0, 0]} /><Bar dataKey="desligamentos" name="Desligamentos" fill="#f87171" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></div><div className="grid grid-cols-2 gap-2 border-t border-surface-border pt-3 text-sm"><div className="rounded-lg bg-surface-card p-2"><p className="text-xs text-gray-400">Total admissões</p><p className="mt-1 text-lg font-bold text-emerald-300">{totalAdmissoes}</p></div><div className="rounded-lg bg-surface-card p-2"><p className="text-xs text-gray-400">Total desligamentos</p><p className="mt-1 text-lg font-bold text-rose-300">{totalDesligamentos}</p></div></div></section>
      <section className="rounded-2xl border border-surface-border bg-surface p-5 shadow-panel"><div className="flex items-center justify-between"><h3 className="text-sm font-bold">Despesas mensais</h3><Wallet size={16} className="text-amber-300" /></div><div className="mt-4 h-56"><ResponsiveContainer width="100%" height="100%"><LineChart data={serie}><CartesianGrid vertical={false} stroke="#2e405d" strokeDasharray="3 4" /><XAxis dataKey="mes" tick={{ fill: '#8fa5c5', fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis hide /><Tooltip formatter={(value: number) => value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} contentStyle={{ background: '#1d2b43', border: '1px solid #3b5175', borderRadius: 10 }} /><Line type="monotone" dataKey="despesas" name="Despesas" stroke="#22c55e" strokeWidth={3} dot={{ r: 4, fill: '#22c55e' }} /></LineChart></ResponsiveContainer></div><div className="rounded-lg bg-surface-card p-3"><p className="text-xs text-gray-400">Total no período</p><p className="mt-1 text-xl font-bold text-amber-300">{totalDespesas.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p></div></section>
      <section className="rounded-2xl border border-surface-border bg-surface p-5 shadow-panel"><div className="flex items-center justify-between"><h3 className="text-sm font-bold">Colaboradores</h3><span className="rounded-md border border-surface-border bg-surface-card px-2 py-1 text-[11px] text-gray-300">Status</span></div>{distribuicao.length ? <><div className="mt-2 h-48"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={distribuicao} dataKey="value" nameKey="name" innerRadius={52} outerRadius={76} paddingAngle={3}>{distribuicao.map(item => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip contentStyle={{ background: '#1d2b43', border: '1px solid #3b5175', borderRadius: 10 }} /></PieChart></ResponsiveContainer></div><div className="space-y-1.5">{distribuicao.map(item => <div key={item.name} className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 text-gray-300"><i className="h-2.5 w-2.5 rounded-full" style={{ background: item.color }} />{item.name}</span><strong>{item.value}</strong></div>)}</div></> : <p className="py-20 text-center text-sm text-gray-400">Ainda não há dados para o gráfico.</p>}<div className="mt-3 rounded-lg bg-surface-card p-3"><p className="text-xs text-gray-400">Taxa de turnover</p><p className="mt-1 text-xl font-bold text-brand-300">{turnover}%</p></div></section>
    </div>
  </section>
}
...
function PortalWeb() {
      ...
      if (data.perfil === 'supervisor') {
        ...
        if (erroObras || erroColaboradores || erroLancamentos || erroAutorizacoes || erroNotas) {
          ...
          const meses = Array.from({ length: 6 }, (_, indice) => {
            const dataMes = new Date(); dataMes.setMonth(dataMes.getMonth() - (5 - indice), 1)
            const chave = dataMes.toISOString().slice(0, 7)
            return { chave, mes: dataMes.toLocaleDateString('pt-BR', { month: 'short' }).replace('.', ''), admissoes: 0, desligamentos: 0, despesas: 0 }
          })
          for (const colaborador of colaboradores ?? []) {
            const admissao = meses.find(item => item.chave === colaborador.data_admissao?.slice(0, 7))
            const desligamento = meses.find(item => item.chave === colaborador.data_demissao?.slice(0, 7))
            if (admissao) admissao.admissoes += 1
            if (desligamento) desligamento.desligamentos += 1
          }
          for (const lancamento of lancamentos ?? []) {
            const mes = meses.find(item => item.chave === lancamento.data?.slice(0, 7))
            if (mes && lancamento.tipo === 'despesa' && lancamento.status !== 'cancelado') mes.despesas += Number(lancamento.valor)
          }
  ...
  return <div className="flex h-screen overflow-hidden bg-background text-white">
    ...
    <div className="flex min-w-0 flex-1 flex-col overflow-hidden"><header className="flex h-12 shrink-0 items-center justify-between border-b border-surface-border bg-[#1f2d46] px-3 md:px-5"><div className="flex items-center gap-3"><button className="rounded-lg p-2 text-gray-300 hover:bg-surface-hover md:hidden" aria-label="Abrir menu" onClick={() => setMenuAberto(aberto => !aberto)}>{menuAberto ? <X size={20} /> : <Menu size={20} />}</button><span className="hidden text-xs text-gray-500 md:block">ADM PRO</span></div><div className="flex items-center gap-3"><div className="hidden w-52 items-center gap-2 rounded-lg border border-[#40506d] bg-[#263550] px-3 py-1.5 text-xs text-gray-400 md:flex"><Search size={13} />Buscar…</div><button className="relative rounded-lg p-2 text-gray-300 hover:bg-surface-hover" aria-label="Notificações"><Bell size={15} /><span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-red-400" /></button><span className="hidden max-w-48 truncate text-xs text-gray-300 lg:block">{perfil.email}</span></div></header><main className="flex-1 overflow-y-auto bg-[#222321] p-4 md:p-5">
      ...
      {perfil.perfil === 'supervisor' && pagina === 'supervisor' && resumoSupervisor && <GraficosSupervisor resumo={resumoSupervisor} serie={serieSupervisor} />}
