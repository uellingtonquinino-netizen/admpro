...
function PortalWeb() {
  ...
  const [pagina, setPagina] = useState<'inicio' | 'financeiro' | 'rh' | 'estoque' | 'supervisor' | 'pessoal'>('inicio')
  ...
  async function carregarPerfil(sessao: Session | null) {
    ...
    if (error) setErro(`Não foi possível carregar seu perfil: ${error.message}`)
      ...
      setPerfil(data)
      setPagina(data.perfil === 'almoxarife' ? 'estoque' : data.perfil === 'supervisor' ? 'supervisor' : data.perfil === 'setor_pessoal' ? 'pessoal' : 'inicio')
  ...
  const navegar = (destino: 'inicio' | 'financeiro' | 'rh' | 'estoque' | 'supervisor' | 'pessoal') => { setPagina(destino); setMenuAberto(false) }
  ...
  return <div className="flex h-screen overflow-hidden bg-background text-white">
    ...
    <div className="flex min-w-0 flex-1 flex-col overflow-hidden"><header className="flex h-[64px] shrink-0 items-center justify-between border-b border-surface-border bg-surface px-4 md:h-[73px] md:px-6"><div className="flex items-center gap-3"><button className="rounded-lg p-2 text-gray-300 hover:bg-surface-hover md:hidden" aria-label="Abrir menu" onClick={() => setMenuAberto(aberto => !aberto)}>{menuAberto ? <X size={20} /> : <Menu size={20} />}</button><div><p className="text-xs text-gray-500">ADM PRO WEB</p><h1 className="text-lg font-semibold">{pagina === 'supervisor' ? 'Painel do Supervisor' : pagina === 'pessoal' ? 'Setor Pessoal' : pagina === 'inicio' ? 'Painel Inicial' : pagina === 'rh' ? 'Colaboradores' : pagina === 'estoque' ? 'Estoque' : 'Lançamentos'}</h1></div></div><p className="hidden text-sm text-gray-400 sm:block">{perfil.email}</p></header><main className="flex-1 overflow-y-auto p-4 md:p-6">
      ...
      {pagina === 'rh' && <section className="mt-7">
        ...
        <input className="mb-4 w-full max-w-md rounded-md border border-surface-border bg-surface px-3 py-2 text-sm text-white" placeholder="Buscar por nome, função ou setor…" value={buscaColaborador} onChange={e => setBuscaColaborador(e.target.value)} />
