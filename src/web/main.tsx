...
function GraficosSupervisor({
  resumo,
  serie,
}: {
  resumo: ResumoSupervisor;
  serie: SerieSupervisor[];
}) {
  const totalAdmissoes = serie.reduce(
    (total, item) => total + item.admissoes,
    0,
  );
  const totalDesligamentos = serie.reduce(
    (total, item) => total + item.desligamentos,
    0,
  );
  const totalDespesas = serie.reduce((total, item) => total + item.despesas, 0);
  const turnover = resumo.colaboradores
    ? Math.round(
        ((totalAdmissoes + totalDesligamentos) / resumo.colaboradores) * 100,
      )
    : 0;
  const distribuicao = [
    { name: "Ativos", value: resumo.colaboradores, color: "#22c55e" },
    {
      name: "Movimentações",
      value: totalAdmissoes + totalDesligamentos,
      color: "#3b82f6",
    },
    { name: "Pendências", value: resumo.pendencias, color: "#f59e0b" },
  ...
  return (
    <section className="mx-auto max-w-7xl pb-7">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <section className="rounded-2xl border border-surface-border bg-surface p-5 shadow-panel">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold">Admissões e desligamentos</h3>
            <span className="rounded-md border border-surface-border bg-surface-card px-2 py-1 text-[11px] text-gray-300">
              Últimos 6 meses
            </span>
          </div>
          <div className="mt-4 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={serie} barGap={5}>
                <CartesianGrid
                  vertical={false}
                  stroke="#2e405d"
                  strokeDasharray="3 4"
          ...
          <div className="grid grid-cols-2 gap-2 border-t border-surface-border pt-3 text-sm">
            <div className="rounded-lg bg-surface-card p-2">
              <p className="text-xs text-gray-400">Total admissões</p>
              <p className="mt-1 text-lg font-bold text-emerald-300">
                {totalAdmissoes}
              </p>
            </div>
            <div className="rounded-lg bg-surface-card p-2">
              <p className="text-xs text-gray-400">Total desligamentos</p>
        ...
        <section className="rounded-2xl border border-surface-border bg-surface p-5 shadow-panel">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold">Despesas mensais</h3>
        ...
        <section className="rounded-2xl border border-surface-border bg-surface p-5 shadow-panel">
          ...
          <div className="mt-3 rounded-lg bg-surface-card p-3">
            <p className="text-xs text-gray-400">Taxa de turnover</p>
            <p className="mt-1 text-xl font-bold text-brand-300">{turnover}%</p>
          </div>
        </section>
      </div>
    </section>
...
function PortalWeb() {
  ...
  const [resumoSupervisor, setResumoSupervisor] =
    useState<ResumoSupervisor | null>(null);
  const [serieSupervisor, setSerieSupervisor] = useState<SerieSupervisor[]>([]);
  const [solicitacoesPessoal, setSolicitacoesPessoal] = useState<
    SolicitacaoPessoal[]
  >([]);
  const [resumoCentral, setResumoCentral] = useState<ResumoCentral[]>([]);
  const [itensCentralPendentes, setItensCentralPendentes] = useState<
    ItemCentral[]
  >([]);
  const [itensSupervisorPendentes, setItensSupervisorPendentes] = useState<
    ItemCentral[]
  >([]);
  const [itemCentralProcessando, setItemCentralProcessando] = useState<
    string | null
  >(null);
  const [itemFinanceiroProcessando, setItemFinanceiroProcessando] = useState<
    string | null
  >(null);
  const [documentoFinanceiroTipo, setDocumentoFinanceiroTipo] = useState<
    "ap" | "nf"
  >("ap");
  ...
  const [carregandoAnexosFinanceiros, setCarregandoAnexosFinanceiros] =
    useState(false);
  const [enviandoAnexosFinanceiros, setEnviandoAnexosFinanceiros] =
    useState(false);
  ...
  const [editandoFornecedorId, setEditandoFornecedorId] = useState<
    number | null
  >(null);
  const [itensLoteSelecionados, setItensLoteSelecionados] = useState<
    Set<string>
  >(new Set());
  ...
  const [formUsuarioMaster, setFormUsuarioMaster] = useState({
    nome: "",
    email: "",
    senha: "",
    perfil: "admin",
  ...
  async function carregarPerfil(sessao: Session | null) {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error)
      setErro(`Não foi possível carregar seu perfil: ${error.message}`);
    else if (!data || !data.ativo)
      setErro("Sua conta não está vinculada a um usuário ativo do sistema.");
    else {
      setPerfil(data);
      setCarimboUrl(data.carimbo_url ?? "");
      setPagina(
        data.perfil === "almoxarife"
          ? "almox"
          : data.perfil === "supervisor"
            ? "supervisor"
            : data.perfil === "setor_pessoal"
              ? "pessoal"
              : data.perfil === "central"
                ? "central"
                : "inicio",
      ...
      if (data.perfil === "master") {
        const [obras, usuarios] = await Promise.all([
          supabase
            .from("empresas")
            .select("id,nome,titulo_obra,estado")
            .order("nome"),
          supabase
            .from("usuarios")
        ...
        if (!obras.error && !usuarios.error) {
          ...
          setResumoMaster({
            ...
            usuarios: ativos.length,
            supervisores: ativos.filter((item) => item.perfil === "supervisor")
              .length,
            administradores: ativos.filter((item) => item.perfil === "admin")
              .length,
          });
      ...
      if (data.perfil === "supervisor") {
        ...
        const [
          ...
          { data: autorizacoes, error: erroAutorizacoes },
          { data: notas, error: erroNotas },
          { data: lotes, error: erroLotes },
        ] = await Promise.all([
          supabase
            .from("empresas")
            .select("id,nome,titulo_obra,estado")
          ...
          supabase
            .from("colaboradores")
            .select("status,nascimento,data_admissao,data_demissao")
          ...
          supabase
            ...
            .select(
              "id,empresa_id,lote_id,beneficiario_nome,valor,aprovado_supervisor_por",
            )
            .in("empresa_id", empresaIds),
          supabase
            .from("notas_fiscais")
            .select(
              "id,empresa_id,lote_id,fornecedor_nome,numero_nf,aprovado_supervisor_por",
            )
            .in("empresa_id", empresaIds),
          supabase
        ...
        if (
          ...
          const idades = ativos
            ...
            .map(
              (item) =>
                (Date.now() -
                  new Date(`${item.nascimento}T00:00:00`).getTime()) /
                31557600000,
            );
          ...
          for (const colaborador of colaboradores ?? []) {
            const admissao = meses.find(
              (item) => item.chave === colaborador.data_admissao?.slice(0, 7),
            );
            const desligamento = meses.find(
              (item) => item.chave === colaborador.data_demissao?.slice(0, 7),
            );
            if (admissao) admissao.admissoes += 1;
            if (desligamento) desligamento.desligamentos += 1;
          }
          for (const lancamento of lancamentos ?? []) {
            const mes = meses.find(
              (item) => item.chave === lancamento.data?.slice(0, 7),
            );
            if (
              mes &&
              ...
              mes.despesas += Number(lancamento.valor);
          }
          setSerieSupervisor(meses.map(({ chave: _chave, ...item }) => item));
          setResumoSupervisor({
            obras: obras ?? [],
            colaboradores: ativos.length,
            idadeMedia: idades.length
              ? Math.round(
                  idades.reduce((total, idade) => total + idade, 0) /
                    idades.length,
                )
              : null,
            ...
            pendencias:
              (autorizacoes ?? []).filter(
                (item) =>
                  item.lote_id !== null &&
                  item.aprovado_supervisor_por === null,
              ).length +
              (notas ?? []).filter(
                (item) =>
                  item.lote_id !== null &&
          ...
          setItensSupervisorPendentes([
            ...(autorizacoes ?? [])
              ...
              .map((item) => ({
                ...
                referencia: "Autorização de pagamento",
                valor: item.valor,
              })),
            ...(notas ?? [])
              .filter(
              ...
              .map((item) => ({
                ...
                referencia: `Nota fiscal ${item.numero_nf ?? "—"}`,
                valor: null,
              })),
          ]);
          setLotesWeb(lotes ?? []);
          ...
          setAutorizacoesWeb(
            (autorizacoes ?? []).map((item) => ({
              ...
              vencimento: null,
              aprovado_por: item.aprovado_supervisor_por,
              lote_id: item.lote_id,
            })),
          );
          setNotasFiscaisWeb(
      ...
      if (data.perfil === "setor_pessoal") {
        const [
          ...
          supabase
            .from("solicitacoes_pessoal")
            .select(
              "id,empresa_id,colaborador_id,tipo,status,solicitado_por,solicitado_em,respondido_por",
            )
        ...
        if (erroSolicitacoes || erroColaboradores || erroEmpresas) {
          setErro(
            "Não foi possível carregar as solicitações do Setor Pessoal.",
          );
        } else {
          const colaboradorPorId = new Map(
            (colaboradores ?? []).map((item) => [item.id, item.nome]),
          );
          const obraPorId = new Map(
            (empresas ?? []).map((item) => [item.id, item.nome]),
          );
          setSolicitacoesPessoal(
            (solicitacoes ?? []).map((item) => ({
              ...item,
              colaborador_nome:
                colaboradorPorId.get(item.colaborador_id) ??
                "Colaborador não encontrado",
              obra_nome:
                obraPorId.get(item.empresa_id) ?? "Obra não encontrada",
            })),
          );
        }
        setCarregando(false);
      ...
      if (data.perfil === "central") {
        const [
          { data: supervisores, error: erroSupervisores },
          ...
          supabase
            .from("autorizacoes_pagamento")
            .select(
              "id,empresa_id,lote_id,beneficiario_nome,valor,aprovado_supervisor_por,aprovado_central_por",
            ),
          supabase
            .from("notas_fiscais")
            .select(
              "id,empresa_id,lote_id,fornecedor_nome,numero_nf,aprovado_supervisor_por,aprovado_central_por",
            ),
        ]);
        if (
          erroSupervisores ||
          ...
          for (const vinculo of vinculos ?? [])
            obrasPorSupervisor.set(vinculo.usuario_id, [
              ...(obrasPorSupervisor.get(vinculo.usuario_id) ?? []),
              vinculo.empresa_id,
            ]);
          const empresaPorLote = new Map(
            (lotes ?? []).map((item) => [item.id, item.empresa_id]),
          );
          ...
          for (const item of [...(autorizacoes ?? []), ...(notas ?? [])]) {
            if (
              item.lote_id === null ||
              item.aprovado_supervisor_por === null ||
              item.aprovado_central_por !== null
            )
              continue;
            const empresaId = empresaPorLote.get(item.lote_id);
            if (empresaId)
              pendenciasPorObra.set(
                empresaId,
                (pendenciasPorObra.get(empresaId) ?? 0) + 1,
              );
          }
          setResumoCentral(
            (supervisores ?? []).map((supervisor) => {
              const obras = (obrasPorSupervisor.get(supervisor.id) ?? [])
                .map((id) => obraPorId.get(id))
                .filter((obra): obra is { id: number; nome: string } => !!obra);
              return {
                id: supervisor.id,
                nome: supervisor.nome,
                email: supervisor.email,
                obras,
                pendencias: obras.reduce(
                  (total, obra) =>
                    total + (pendenciasPorObra.get(obra.id) ?? 0),
                  0,
                ),
              };
            }),
          ...
          setAutorizacoesWeb(
            (autorizacoes ?? []).map((item) => ({
              ...
              valor: Number(item.valor),
              vencimento: null,
              aprovado_por: item.aprovado_central_por,
              lote_id: item.lote_id,
            })),
          );
          setNotasFiscaisWeb(
      ...
      const [colaboradores, lancamentos] = await Promise.all([
        supabase
          .from("colaboradores")
          .select("nome,funcao,nascimento,salario_base")
      ...
      if (colaboradores.error || lancamentos.error) {
        ...
        const hoje = new Date();
        const idades = (colaboradores.data ?? [])
          .filter((item) => item.nascimento)
          .map(
            (item) =>
              (hoje.getTime() -
        ...
        for (const item of colaboradores.data ?? []) {
          ...
          const atual = porFuncao.get(item.funcao) ?? {
            quantidade: 0,
            custo: 0,
          };
          atual.quantidade += 1;
          atual.custo += Number(item.salario_base);
          porFuncao.set(item.funcao, atual);
        }
        setResumo({
          ativos: colaboradores.data?.length ?? 0,
          custoFolha: (colaboradores.data ?? []).reduce(
            (total, item) => total + Number(item.salario_base),
            0,
          ),
          lancamentos: lancamentos.data ?? [],
          mediaIdade: idades.length
            ? Math.round(
                idades.reduce((total, idade) => total + idade, 0) /
                  idades.length,
              )
            : null,
          aniversariantes: (colaboradores.data ?? [])
            .filter(
              (item) =>
                item.nascimento?.slice(5, 7) ===
            ...
            .sort((a, b) => a.nascimento.localeCompare(b.nascimento)),
          porFuncao: [...porFuncao]
        ...
        const { data: listaFinanceira, error: erroFinanceiro } = await supabase
          ...
          .limit(100);
        if (erroFinanceiro)
          setErro("Não foi possível carregar os lançamentos financeiros.");
        ...
        const { data: produtos, error: erroProdutos } = await supabase
          .from("produtos")
          .select(
            "id,codigo,nome,unidade,estoque_atual,estoque_minimo,valor_unitario",
          )
          .eq("empresa_id", data.empresa_id)
          .order("nome");
        ...
        const [
          ...
          supabase
            ...
            .select("id,data,numero_nota,fornecedor_nome,valor_total")
            .eq("empresa_id", data.empresa_id)
            .order("data", { ascending: false })
            .order("id", { ascending: false })
            .limit(80),
          supabase
            ...
            .select(
              "id,data,produto_nome,produto_codigo,quantidade,retirado_por_nome,setor",
            )
            .eq("empresa_id", data.empresa_id)
            .order("data", { ascending: false })
            .order("id", { ascending: false })
        ...
        if (erroEntradas || erroSaidas)
          setErro(
            "Não foi possível carregar as movimentações do almoxarifado.",
          );
        else {
          setEntradasWeb(entradas ?? []);
          setSaidasWeb(saidas ?? []);
        }
        const [
          { data: autorizacoes, error: erroAutorizacoes },
          { data: notasFiscais, error: erroNotasFiscais },
        ] = await Promise.all([
          supabase
            .from("autorizacoes_pagamento")
            .select(
              "id,beneficiario_tipo,beneficiario_id,beneficiario_nome,descricao,valor,vencimento,aprovado_por,lote_id",
            )
            .eq("empresa_id", data.empresa_id)
            .order("created_at", { ascending: false })
            .limit(100),
          supabase
            .from("notas_fiscais")
            .select(
              "id,fornecedor_id,fornecedor_nome,numero_nf,data,valor_total,aprovado_por,lote_id",
            )
            .eq("empresa_id", data.empresa_id)
            .order("created_at", { ascending: false })
            .limit(100),
        ]);
        if (erroAutorizacoes || erroNotasFiscais)
          setErro("Não foi possível carregar as autorizações e notas fiscais.");
        else {
          setAutorizacoesWeb(autorizacoes ?? []);
          setNotasFiscaisWeb(notasFiscais ?? []);
        }
        const { data: fornecedores, error: erroFornecedores } = await supabase
          .from("fornecedores")
          .select(
            "id,nome,tipo_pessoa,cnpj,cpf,email,telefone,categoria,forma_pagamento",
          )
          .eq("empresa_id", data.empresa_id)
          .eq("ativo", 1)
          .order("nome");
        ...
        const { data: lotes, error: erroLotes } = await supabase
          ...
          .order("id", { ascending: false });
        if (erroLotes)
          setErro("Não foi possível carregar os lotes financeiros.");
        ...
        if (categorias.error || contas.error)
          setErro("Não foi possível carregar categorias e contas financeiras.");
        else {
          setCategoriasWeb(categorias.data ?? []);
          setContasWeb(contas.data ?? []);
        }
      }
    }
    setCarregando(false);
  ...
  useEffect(() => {
    void supabase.auth
      .getSession()
      .then(({ data }) => carregarPerfil(data.session));
  ...
  async function salvarFornecedor(evento: FormEvent) {
    ...
    const dadosFornecedor = {
      ...
      tipo_pessoa: formFornecedor.tipo_pessoa,
      cnpj:
        formFornecedor.tipo_pessoa === "pj"
          ? formFornecedor.cnpj.trim() || null
          : null,
      cpf:
        formFornecedor.tipo_pessoa === "pf"
          ? formFornecedor.cpf.trim() || null
          : null,
      email: formFornecedor.email.trim() || null,
      telefone: formFornecedor.telefone.trim() || null,
      categoria: formFornecedor.categoria.trim() || null,
      forma_pagamento: formFornecedor.forma_pagamento,
      ativo: 1,
    };
    ...
    if (error) {
      setErro(`Não foi possível cadastrar o fornecedor: ${error.message}`);
      return;
    }
  ...
  async function salvarAp(evento: FormEvent) {
    evento.preventDefault();
    if (!perfil) return;
    const fornecedor = fornecedoresWeb.find(
      (item) => item.id === Number(formAp.fornecedor_id),
    );
    if (!fornecedor) {
      setErro("Selecione o fornecedor da autorização.");
      return;
    }
    setErro("");
    setSalvandoAp(true);
    const dadosAp = {
      empresa_id: perfil.empresa_id,
      beneficiario_tipo: "fornecedor",
      beneficiario_id: fornecedor.id,
      beneficiario_nome: fornecedor.nome,
      descricao: formAp.descricao.trim(),
      observacoes: formAp.observacoes.trim(),
      boletos: [{ valor: Number(formAp.valor), vencimento: formAp.vencimento }],
      solicitante: perfil.nome,
      autorizado_por: perfil.nome,
    };
    const { error } = editandoApId
      ? await supabase.rpc("atualizar_ap", {
          p: { ...dadosAp, id: editandoApId },
        })
      : await supabase.rpc("criar_ap", { p: dadosAp });
    setSalvandoAp(false);
    if (error) {
      setErro(`Não foi possível criar a autorização: ${error.message}`);
      return;
    }
    setNovaAp(false);
    setEditandoApId(null);
    setFormAp({
      fornecedor_id: "",
      descricao: "",
      valor: "",
  ...
  async function salvarNf(evento: FormEvent) {
    ...
    if (!perfil) return;
    const fornecedor = fornecedoresWeb.find(
      (item) => item.id === Number(formNf.fornecedor_id),
    );
    if (!fornecedor) {
      setErro("Selecione o fornecedor da nota fiscal.");
      return;
    }
    setErro("");
    setSalvandoNf(true);
    const dadosNf = {
      ...
      numero_nf: formNf.numero_nf.trim(),
      numero_pedido: formNf.numero_pedido.trim(),
      data: formNf.data,
      data_emissao_nf: formNf.data,
      boletos: [{ valor: Number(formNf.valor), vencimento: formNf.vencimento }],
    };
    const { error } = editandoNfId
      ? await supabase.rpc("atualizar_nota_fiscal", {
          p: { ...dadosNf, id: editandoNfId },
        })
      : await supabase.rpc("criar_nota_fiscal", { p: dadosNf });
    setSalvandoNf(false);
    if (error) {
      setErro(`Não foi possível criar a nota fiscal: ${error.message}`);
      return;
    }
    setNovaNf(false);
    setEditandoNfId(null);
    setFormNf({
      fornecedor_id: "",
      numero_nf: "",
      numero_pedido: "",
  ...
  async function criarLoteFinanceiro() {
    if (!perfil || itensLoteSelecionados.size === 0) {
      setErro("Selecione ao menos uma AP ou Nota Fiscal aprovada.");
      return;
    }
    const apIds = [...itensLoteSelecionados]
      .filter((item) => item.startsWith("ap-"))
      .map((item) => Number(item.slice(3)));
    const nfIds = [...itensLoteSelecionados]
      .filter((item) => item.startsWith("nf-"))
      .map((item) => Number(item.slice(3)));
    setErro("");
    setProcessandoLote(true);
    const { error } = await supabase.rpc("fechar_lote_financeiro", {
      p: {
        empresa_id: perfil.empresa_id,
        ap_ids: apIds,
        nf_ids: nfIds,
    ...
    if (error) {
      setErro(`Não foi possível criar o lote: ${error.message}`);
      return;
    }
  ...
  async function enviarLoteFinanceiro(loteId: number) {
    ...
    setErro("");
    setProcessandoLote(true);
    const { error } = await supabase.rpc("enviar_lotes_supervisor", {
      p_lote_ids: [loteId],
    });
    setProcessandoLote(false);
    if (error) {
      setErro(`Não foi possível enviar o lote: ${error.message}`);
      return;
    }
    await carregarPerfil(session);
  }
  ...
  async function salvarEntrada(evento: FormEvent) {
    evento.preventDefault();
    if (!perfil) return;
    const produto = produtosWeb.find(
      (item) => item.id === Number(formEntrada.produto_id),
    );
    if (!produto) {
      setErro("Selecione o material para registrar a entrada.");
      return;
    }
    setErro("");
    setSalvandoEntrada(true);
    const { error } = await supabase.rpc("criar_entrada_almoxarifado", {
      p: {
        empresa_id: perfil.empresa_id,
        ...
        fornecedor_nome:
          formEntrada.fornecedor_nome.trim() || "Fornecedor não informado",
        valor_desconto: 0,
        itens: [
          {
            ...
            valor_unitario: Number(formEntrada.valor_unitario),
          },
        ],
      },
    });
    ...
    if (error) {
      setErro(`Não foi possível registrar a entrada: ${error.message}`);
      return;
    }
  ...
  async function salvarSaida(evento: FormEvent) {
    ...
    if (!perfil) return;
    const produto = produtosWeb.find(
      (item) => item.id === Number(formSaida.produto_id),
    );
    if (!produto) {
      setErro("Selecione o material para registrar a saída.");
      return;
    }
    setErro("");
    setSalvandoSaida(true);
    const { error } = await supabase.rpc("criar_saida_almoxarifado", {
      p: {
        empresa_id: perfil.empresa_id,
        data: formSaida.data,
        produto_id: produto.id,
    ...
    if (error) {
      setErro(`Não foi possível registrar a saída: ${error.message}`);
      return;
    }
    setNovaSaida(false);
    setFormSaida({
  ...
  async function salvarUsuarioMaster(evento: FormEvent) {
    ...
    setErro("");
    setSalvandoUsuarioMaster(true);
    const { data, error } = await supabase.functions.invoke("usuarios-admin", {
      body: {
        acao: "criar",
        nome: formUsuarioMaster.nome.trim(),
        email: formUsuarioMaster.email.trim(),
        senha: formUsuarioMaster.senha,
        perfil: formUsuarioMaster.perfil,
        empresa_id: Number(formUsuarioMaster.empresa_id),
      },
    ...
    if (error || data?.error) {
      setErro(
        `Não foi possível criar o usuário: ${data?.error ?? error?.message ?? "erro desconhecido"}`,
      );
      return;
    }
    setNovoUsuarioMaster(false);
    setFormUsuarioMaster({
      nome: "",
      email: "",
      senha: "",
      perfil: "admin",
  ...
  async function responderSolicitacaoPessoal(
    evento: FormEvent,
    solicitacao: SolicitacaoPessoal,
  ) {
    evento.preventDefault();
    ...
    setErro("");
    setSalvandoRespostaPessoal(true);
    try {
      for (const [ordem, arquivo] of anexosResposta.entries()) {
        ...
        const { error: erroAnexo } = await supabase
          .from("solicitacoes_pessoal_anexos")
          .insert({
            solicitacao_id: solicitacao.id,
            caminho: `supabase://documentos-rh/${caminho}`,
            nome: arquivo.name,
            origem: "setor_pessoal",
            ordem,
          });
      ...
      const { error: erroResposta } = await supabase
        ...
        .update({
          status: "respondido",
          respondido_por: perfil.nome,
          resposta_observacoes: respostaPessoal.trim() || null,
          respondido_em: new Date().toISOString(),
        })
        .eq("id", solicitacao.id);
      if (erroResposta) throw erroResposta;
      setSolicitacaoRespondendoId(null);
      setRespostaPessoal("");
      setAnexosResposta([]);
      await carregarPerfil(session);
    } catch (causa) {
      setErro(
        `Não foi possível enviar a resposta: ${causa instanceof Error ? causa.message : "erro desconhecido"}`,
      );
    } finally {
  ...
  async function aprovarItemPendente(item: ItemCentral) {
    ...
    setErro("");
    setItemCentralProcessando(chave);
    const { error } = await supabase.rpc(
      item.tipo === "ap" ? "aprovar_ap" : "aprovar_nota_fiscal",
      { p_id: item.id },
    );
    setItemCentralProcessando(null);
    if (error) {
      setErro(`Não foi possível aprovar o item: ${error.message}`);
      return;
    }
    await carregarPerfil(session);
  }

  async function aprovarItemCentral(item: ItemCentral) {
    await aprovarItemPendente(item);
  }
  ...
  async function aprovarItemFinanceiro(tipo: "ap" | "nf", id: number) {
    ...
    const chave = `${tipo}-${id}`;
    setErro("");
    setItemFinanceiroProcessando(chave);
    const { error } = await supabase.rpc(
      tipo === "ap" ? "aprovar_ap" : "aprovar_nota_fiscal",
      { p_id: id },
    );
  ...
  function caminhoStorage(caminho: string) {
    return caminho.startsWith("supabase://documentos-rh/")
      ? caminho.slice("supabase://documentos-rh/".length)
      : caminho;
  }

  async function carregarAnexosFinanceiros(
    tipo = documentoFinanceiroTipo,
    idTexto = documentoFinanceiroId,
  ) {
    if (!idTexto) {
    ...
    const resultado =
      tipo === "ap"
        ? await supabase
            ...
            .eq("ap_id", Number(idTexto))
            .order("ordem")
        : await supabase
            .from("notas_fiscais_anexos")
            .select("id,caminho,categoria,ordem")
            .eq("nota_id", Number(idTexto))
            .order("ordem");
    setCarregandoAnexosFinanceiros(false);
    if (resultado.error) {
      setErro(
        `Não foi possível carregar os anexos: ${resultado.error.message}`,
      );
      return;
    }
  ...
  async function enviarAnexosFinanceiros(evento: FormEvent) {
    ...
    if (
      ...
      novosAnexosFinanceiros.length === 0
    ) {
      setErro("Selecione o documento e ao menos um arquivo.");
      return;
    }
    if (!["admin", "master"].includes(perfil.perfil)) {
      setErro(
        "Seu perfil possui acesso apenas para consulta de anexos financeiros.",
      );
      return;
    }
    setErro("");
    setEnviandoAnexosFinanceiros(true);
    try {
      ...
      setErro(
        `Não foi possível enviar os anexos: ${causa instanceof Error ? causa.message : "erro desconhecido"}`,
      );
    } finally {
  ...
  async function obterUrlArquivo(caminho: string) {
    const { data, error } = await supabase.storage
      .from("documentos-rh")
      .createSignedUrl(caminhoStorage(caminho), 60);
    if (error || !data?.signedUrl)
      throw (
        error ??
        new Error("Não foi possível gerar o acesso temporário ao arquivo.")
      );
  ...
  async function visualizarArquivo(caminho: string) {
    try {
      window.open(await obterUrlArquivo(caminho), "_blank", "noopener");
    } catch (causa) {
      setErro(
        `Não foi possível abrir o arquivo: ${causa instanceof Error ? causa.message : "erro desconhecido"}`,
      );
    }
  }
  ...
  async function baixarArquivo(caminho: string) {
    try {
      ...
      link.href = objeto;
      link.download = caminhoStorage(caminho).split("/").at(-1) ?? "documento";
      link.click();
      URL.revokeObjectURL(objeto);
    } catch (causa) {
      setErro(
        `Não foi possível baixar o arquivo: ${causa instanceof Error ? causa.message : "erro desconhecido"}`,
      );
    }
  }
  ...
  async function imprimirArquivo(caminho: string) {
    try {
      ...
      if (!janela)
        throw new Error("Permita pop-ups para imprimir o documento.");
      ...
      setErro(
        `Não foi possível imprimir o arquivo: ${causa instanceof Error ? causa.message : "erro desconhecido"}`,
      );
    }
  }
  ...
  async function assinarPdfNoStorage(caminho: string) {
    ...
    if (!documentoFinanceiroId) {
      setErro("Selecione a AP ou a Nota Fiscal para assinar.");
      return;
    }
    try {
      const resposta = await fetch(await obterUrlArquivo(caminho));
      if (!resposta.ok)
        throw new Error("Não foi possível abrir o PDF original.");
      const pdf = await PDFDocument.load(await resposta.arrayBuffer());
      const dadosAssinatura = await fetch(carimboUrl).then((r) =>
        r.arrayBuffer(),
      );
      let imagem;
      try {
        imagem = await pdf.embedPng(dadosAssinatura);
      } catch {
        imagem = await pdf.embedJpg(dadosAssinatura);
      }
      ...
      pagina.drawRectangle({
        x: x - 8,
        y: 20,
        width: assinaturaLargura + 16,
        height: assinaturaAltura + 32,
        color: rgb(1, 1, 1),
        opacity: 0.92,
      ...
      pagina.drawText(
        `Aprovado por ${perfil.nome} em ${new Date().toLocaleString("pt-BR")}`,
        { x, y: 28, size: 7, color: rgb(0.08, 0.18, 0.36) },
      );
      const arquivoOriginal =
        caminhoStorage(caminho)
          .split("/")
          .at(-1)
          ?.replace(/\.pdf$/i, "") ?? "documento";
      ...
      const bufferPdf = bytesPdf.buffer.slice(
        bytesPdf.byteOffset,
        bytesPdf.byteOffset + bytesPdf.byteLength,
      ) as ArrayBuffer;
      const { error: erroUpload } = await supabase.storage
        .from("documentos-rh")
        .upload(destino, new Blob([bufferPdf], { type: "application/pdf" }), {
          contentType: "application/pdf",
          upsert: false,
        });
      if (erroUpload) throw erroUpload;
      const { error: erroRegistro } = await supabase.rpc(
        "registrar_pdf_assinado_web",
        {
          p_tipo: documentoFinanceiroTipo,
          p_id: Number(documentoFinanceiroId),
          p_caminho: `supabase://documentos-rh/${destino}`,
        },
      );
      if (erroRegistro) throw erroRegistro;
      ...
      setErro(
        `Não foi possível aplicar a assinatura: ${causa instanceof Error ? causa.message : "erro desconhecido"}`,
      );
    }
  }
  ...
  function selecionarCarimbo(arquivo: File | undefined) {
    ...
    leitor.onload = () => {
      ...
      imagem.onload = () => {
        ...
        quadro.width = imagem.width * escala;
        quadro.height = imagem.height * escala;
        quadro
          .getContext("2d")
          ?.drawImage(imagem, 0, 0, quadro.width, quadro.height);
  ...
  async function salvarCarimbo() {
    ...
    const { error } = await supabase.rpc("atualizar_meu_carimbo", {
      p_carimbo_url: carimboUrl || null,
    });
    setSalvandoCarimbo(false);
    if (error) {
      setErro(`Não foi possível salvar a assinatura: ${error.message}`);
      return;
    }
    setPerfil((atual) =>
      atual ? { ...atual, carimbo_url: carimboUrl || null } : atual,
    );
  }
  ...
  if (!session || !perfil) {
    return (
      <main className="min-h-screen grid place-items-center bg-surface px-4">
        <form
          onSubmit={entrar}
          className="w-full max-w-sm rounded-xl border border-surface-border bg-surface-card p-8 space-y-4"
        >
          ...
          {erro && (
            <p className="rounded-md bg-red-950/50 p-3 text-sm text-red-300">
              {erro}
            </p>
          )}
          <button
  ...
  const colaboradoresFiltrados = colaboradoresWeb.filter((item) => {
    const correspondeBusca =
      `${item.nome} ${item.funcao ?? ""} ${item.setor ?? ""}`
        .toLowerCase()
        .includes(buscaColaborador.toLowerCase());
    return (
      correspondeBusca &&
      (filtroStatusRh === "todos" || item.status === filtroStatusRh)
    );
  });
  const resumoStatusRh = {
    ...
    afastados: colaboradoresWeb.filter((item) => item.status === "afastado")
      .length,
  };
  const lancamentosFiltrados = lancamentos.filter((item) => {
    const correspondeBusca = `${item.descricao} ${item.status} ${item.data}`
      .toLowerCase()
      .includes(buscaLancamento.toLowerCase());
    return (
      correspondeBusca &&
      (filtroTipoFinanceiro === "todos" || item.tipo === filtroTipoFinanceiro)
    );
  });
  ...
  const produtosFiltrados = produtosWeb.filter((item) =>
    `${item.codigo} ${item.nome} ${item.unidade ?? ""}`
      .toLowerCase()
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-white">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-[200px] shrink-0 flex-col border-r border-surface-border bg-[#1f2d46] px-3 py-4 transition-transform md:relative md:translate-x-0 ${menuAberto ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="mb-7 flex items-center gap-2.5 px-1">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold">ADM PRO</p>
            <p className="truncate text-[10px] text-gray-400">
              {perfil.perfil === "admin"
                ? "Gestão da obra"
                : (nomesPerfil[perfil.perfil] ?? "Portal web")}
            </p>
          </div>
        </div>
        <nav className="flex-1 space-y-1">
          {perfil.perfil === "master" && (
            <button className="flex w-full items-center gap-3 rounded-xl bg-brand-600 px-3 py-2.5 text-sm font-semibold shadow-glow-sm">
              <Building2 size={16} />
          ...
          {perfil.perfil === "setor_pessoal" && (
            <button
              ...
              onClick={() => navegar("pessoal")}
            >
              <UsersRound size={16} />
              Solicitações
            </button>
          )}
          {perfil.perfil === "central" && (
            <button
              className={
                pagina === "central"
                  ? "flex w-full items-center gap-3 rounded-xl bg-brand-600 px-3 py-2.5 text-sm font-semibold shadow-glow-sm"
          ...
          {perfil.perfil === "admin" && (
            <>
              <p className="px-3 pb-1 text-[11px] font-medium uppercase tracking-wide text-gray-500">
                Recursos Humanos
              </p>
              <button
                className={
                  pagina === "rh"
                    ? "flex w-full items-center gap-3 rounded-lg bg-brand-600 px-3 py-2 text-sm font-medium shadow-glow-sm"
                    : "flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-300 hover:bg-surface-hover"
                }
                onClick={() => navegar("rh")}
              >
                <UsersRound size={15} />
        ...
        <div className="border-t border-surface-border pt-4">
          <div className="flex items-center gap-2.5 px-2">
            <div className="grid h-7 w-7 place-items-center rounded-full bg-brand-500/10 text-xs font-bold text-brand-400">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[11px] text-gray-500">
                {nomesPerfil[perfil.perfil] ?? perfil.perfil}
              </p>
            </div>
            <button
              title="Sair"
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex h-12 shrink-0 items-center justify-between border-b border-surface-border bg-[#1f2d46] px-3 md:px-5">
          <div className="flex items-center gap-3">
            <button
              className="rounded-lg p-2 text-gray-300 hover:bg-surface-hover md:hidden"
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden max-w-48 truncate text-xs text-gray-300 lg:block">
              {perfil.email}
            </span>
          </div>
        </header>
        <main className="flex-1 overflow-y-auto bg-[#222321] p-4 md:p-5">
          {erro && (
            <p className="mt-6 rounded-md bg-red-950/50 p-3 text-sm text-red-300">
              {erro}
            </p>
          )}
          {perfil.perfil === "central" && (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-400">
                      Itens já aprovados pelo Supervisor e prontos para decisão
                      do Escritório Central.
                    </p>
                  </div>
                  <span className="rounded-full bg-amber-500/15 px-3 py-1.5 text-sm text-amber-300">
                    {itensCentralPendentes.length} pendente(s)
                  </span>
                </div>
                <div className="mt-4 divide-y divide-surface-border">
                  {itensCentralPendentes.length === 0 ? (
                    <p className="py-5 text-sm text-gray-400">
                      Não há itens aguardando aprovação.
                    </p>
                  ) : (
                    itensCentralPendentes.map((item) => (
                      <article
                      ...
                      >
                        ...
                        <button
                          className="w-fit rounded-lg bg-brand-600 px-3 py-2 text-sm font-medium disabled:opacity-60"
                          disabled={
                            itemCentralProcessando === `${item.tipo}-${item.id}`
                          }
                          onClick={() => void aprovarItemCentral(item)}
                        >
                          {itemCentralProcessando === `${item.tipo}-${item.id}`
                            ? "Aprovando…"
                            : "Aprovar"}
          ...
          {perfil.perfil === "setor_pessoal" && (
            <section className="mx-auto mt-6 max-w-7xl">
              <div className="rounded-2xl border border-surface-border bg-surface p-5">
                ...
                <p className="mt-1 text-sm text-gray-400">
                  Anexe os documentos de retorno antes de concluir a resposta.
                </p>
                <div className="mt-4 space-y-3">
                  {solicitacoesPessoal.filter(
                    (item) => item.status === "pendente",
                  ).length === 0 ? (
                    <p className="text-sm text-gray-400">
                      Não há solicitações aguardando resposta.
                    </p>
                  ) : (
                    solicitacoesPessoal
                      ...
                      .map((item) => (
                        ...
                        >
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 truncate text-sm text-gray-400">
                                {tiposSolicitacao[item.tipo] ?? item.tipo} ·{" "}
                                {item.obra_nome}
                              </p>
                            </div>
                            <button
                              className="w-fit rounded-lg border border-brand-500/40 px-3 py-2 text-sm text-brand-300 hover:bg-brand-500/10"
                              onClick={() => {
                                setSolicitacaoRespondendoId((aberto) =>
                                  aberto === item.id ? null : item.id,
                                );
                                setRespostaPessoal("");
                                setAnexosResposta([]);
                              }}
                            >
                              {solicitacaoRespondendoId === item.id
                                ? "Cancelar"
                                : "Responder"}
                            </button>
                          </div>
                          {solicitacaoRespondendoId === item.id && (
                            <form
                              onSubmit={(evento) =>
                                void responderSolicitacaoPessoal(evento, item)
                              }
                              className="mt-4 grid gap-3 border-t border-surface-border pt-4"
                            >
                              <label className="text-sm text-gray-300">
                                Observações
                                <textarea
                                  className="mt-1 min-h-24 w-full rounded-lg border border-surface-border bg-surface px-3 py-2 text-white"
                                  value={respostaPessoal}
                                  onChange={(e) =>
                                    setRespostaPessoal(e.target.value)
                                  }
                                  placeholder="Informe os documentos e orientações para a obra."
                                />
                              </label>
                              <label className="text-sm text-gray-300">
                                Documentos de retorno
                                <input
                                  className="mt-1 block w-full text-sm text-gray-300 file:mr-3 file:rounded-lg file:border-0 file:bg-brand-600 file:px-3 file:py-2 file:text-sm file:font-medium file:text-white"
                                  type="file"
                                  multiple
                                  onChange={(e) =>
                                    setAnexosResposta(
                              ...
                              <div>
                                <button
                                  ...
                                  disabled={salvandoRespostaPessoal}
                                >
                                  {salvandoRespostaPessoal
                                    ? "Enviando…"
                                    : "Enviar resposta"}
                                </button>
                              </div>
          ...
          {perfil.perfil === "master" && (
            <section className="mx-auto mt-6 max-w-7xl">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm text-gray-400">
                  Gerencie os acessos do sistema a partir deste painel.
                </p>
                <button
                  className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium hover:bg-brand-500"
                  onClick={() => setNovoUsuarioMaster((aberto) => !aberto)}
                >
                  {novoUsuarioMaster ? "Cancelar cadastro" : "+ Novo usuário"}
                </button>
              </div>
              {novoUsuarioMaster && (
                <form
                  onSubmit={salvarUsuarioMaster}
                  className="mt-4 grid gap-3 rounded-2xl border border-surface-border bg-surface p-4 md:grid-cols-2"
                >
                  <label className="text-sm text-gray-300 md:col-span-2">
                    Nome completo
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formUsuarioMaster.nome}
                      onChange={(e) =>
                        setFormUsuarioMaster({
                          ...formUsuarioMaster,
                          nome: e.target.value,
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <select
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formUsuarioMaster.perfil}
                      onChange={(e) =>
                        setFormUsuarioMaster({
                          ...formUsuarioMaster,
                    ...
                    >
                      ...
                      <option value="central">Escritório Central</option>
                      <option value="setor_pessoal">Setor Pessoal</option>
                    </select>
                  </label>
                  <label className="text-sm text-gray-300">
                    Obra
                    <select
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formUsuarioMaster.empresa_id}
                      onChange={(e) =>
                        setFormUsuarioMaster({
                          ...formUsuarioMaster,
                  ...
                  <div className="md:col-span-2">
                    <button
                      className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium disabled:opacity-60"
                      disabled={salvandoUsuarioMaster}
                    >
                      {salvandoUsuarioMaster ? "Criando…" : "Criar usuário"}
                    </button>
                  </div>
                </form>
              )}
          ...
          {perfil.perfil === "master" && resumoMaster && (
            <section className="mx-auto max-w-7xl py-2 md:py-4">
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-gray-400">
                  Visão consolidada da estrutura da empresa e dos acessos do
                  sistema.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4">
                <div className="rounded-2xl border border-brand-500/35 bg-surface p-5">
                ...
                <div className="rounded-2xl border border-emerald-500/35 bg-surface p-5">
                  ...
                  <p className="mt-5 text-4xl font-bold text-emerald-300">
                    {resumoMaster.usuarios}
                  </p>
                  <p className="mt-1 text-sm text-gray-400">
                    com acesso ao sistema
                  </p>
                </div>
                <div className="rounded-2xl border border-purple-500/35 bg-surface p-5">
                  ...
                  <p className="mt-5 text-4xl font-bold text-purple-300">
                    {resumoMaster.supervisores}
                  </p>
                  <p className="mt-1 text-sm text-gray-400">
                    responsáveis por obras
                  </p>
                </div>
                <div className="rounded-2xl border border-amber-500/35 bg-surface p-5">
                  ...
                  <p className="mt-5 text-4xl font-bold text-amber-300">
                    {resumoMaster.administradores}
                  </p>
                  <p className="mt-1 text-sm text-gray-400">
                    gestão operacional
                  </p>
              ...
              <div className="mt-6 grid gap-5 lg:grid-cols-3">
                <section className="rounded-2xl border border-surface-border bg-surface p-5 lg:col-span-2">
                  <h3 className="font-semibold">Estrutura operacional</h3>
                  <p className="mt-1 text-sm text-gray-400">
                    Acesso organizado pelos mesmos núcleos do aplicativo
                    desktop.
                  </p>
                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl bg-surface-hover p-4">
                      ...
                      <p className="mt-1 text-sm text-gray-400">
                        Aprovação e acompanhamento de lotes.
                      </p>
                    </div>
                    <div className="rounded-xl bg-surface-hover p-4">
                      <p className="font-medium">Setor Pessoal</p>
                      <p className="mt-1 text-sm text-gray-400">
                        Fila de solicitações trabalhistas.
                      </p>
                    </div>
                    <div className="rounded-xl bg-surface-hover p-4">
                      <p className="font-medium">Supervisores</p>
                      <p className="mt-1 text-sm text-gray-400">
                        Gestão agrupada por obras vinculadas.
                      </p>
                    </div>
                    <div className="rounded-xl bg-surface-hover p-4">
                      <p className="font-medium">Administração da obra</p>
                      <p className="mt-1 text-sm text-gray-400">
                        RH, financeiro e almoxarifado.
                      </p>
                    </div>
                  </div>
                </section>
                <section className="rounded-2xl border border-surface-border bg-surface p-5">
                  <h3 className="font-semibold">Próxima conferência</h3>
                  <p className="mt-3 text-sm leading-6 text-gray-400">
                    Revise os acessos ativos e os vínculos de supervisores às
                    obras antes de liberar novos usuários.
                  </p>
                  <div className="mt-6 rounded-xl border border-brand-500/25 bg-brand-500/10 p-3 text-sm text-brand-200">
                    A gestão completa de usuários e obras continua sendo migrada
                    para as telas web.
                  </div>
                </section>
              </div>
          ...
          {perfil.perfil === "master" && resumoMaster && (
            <section className="mx-auto -mt-5 max-w-7xl pb-6">
              <div className="grid gap-5 lg:grid-cols-2">
                <section className="overflow-hidden rounded-2xl border border-surface-border bg-surface">
                  <div className="flex items-center justify-between border-b border-surface-border p-5">
                    <div>
                      <h3 className="font-semibold">Obras cadastradas</h3>
                      <p className="mt-1 text-sm text-gray-400">
                        Visão rápida dos empreendimentos.
                      </p>
                    </div>
                    <span className="text-sm text-gray-400">
                      {obrasMaster.length} total
                    </span>
                  </div>
                  <div className="divide-y divide-surface-border">
                    {obrasMaster.slice(0, 6).map((obra) => (
                      <div
                        key={obra.id}
                      ...
                      >
                        ...
                        <span className="rounded-full bg-surface-hover px-2.5 py-1 text-xs text-gray-300">
                          {obra.estado || "Sem UF"}
                        </span>
                      </div>
                    ))}
                    {obrasMaster.length === 0 && (
                ...
                <section className="overflow-hidden rounded-2xl border border-surface-border bg-surface">
                  <div className="flex items-center justify-between border-b border-surface-border p-5">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-400">
                        Contas e perfis cadastrados.
                      </p>
                    </div>
                    <span className="text-sm text-gray-400">
                      {usuariosMaster.length} total
                    </span>
                  </div>
                  <div className="divide-y divide-surface-border">
                    {usuariosMaster.slice(0, 6).map((usuario) => (
                      <div
                        key={usuario.id}
                        className="flex items-center justify-between gap-3 p-4"
                      >
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 truncate text-xs text-gray-400">
                            {usuario.email} ·{" "}
                            {nomesPerfil[usuario.perfil] ?? usuario.perfil}
                          </p>
                        </div>
                        <span
                          className={
                            usuario.ativo
                              ...
                              : "rounded-full bg-gray-500/15 px-2.5 py-1 text-xs text-gray-400"
                          }
                        >
                ...
                <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4">
                  <div className="rounded-2xl border border-emerald-500/35 bg-surface p-5">
                    ...
                    <p className="mt-5 text-3xl font-bold">
                      {resumoSupervisor.obras.length}
                    </p>
                    <p className="mt-1 text-sm text-gray-400">
                      obras · {resumoSupervisor.colaboradores} colaboradores
                    </p>
                  </div>
                  <div className="rounded-2xl border border-brand-500/35 bg-surface p-5">
                    <div className="flex items-center gap-2 text-brand-300">
                    ...
                    <p className="mt-5 text-2xl font-bold text-brand-300">
                      {resumoSupervisor.admissoes} /{" "}
                      {resumoSupervisor.desligamentos}
                    </p>
                    <p className="mt-1 text-sm text-gray-400">
                  ...
                  <div className="rounded-2xl border border-amber-500/35 bg-surface p-5">
                    ...
                    <p className="mt-5 text-2xl font-bold text-amber-300">
                      {resumoSupervisor.despesas.toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL",
                      })}
                    </p>
                  ...
                  <div className="rounded-2xl border border-purple-500/35 bg-surface p-5">
                    ...
                    <p className="mt-5 text-3xl font-bold text-purple-300">
                      {resumoSupervisor.colaboradores}
                    </p>
                    <p className="mt-1 text-sm text-gray-400">
                      idade média:{" "}
                      {resumoSupervisor.idadeMedia
                        ? `${resumoSupervisor.idadeMedia} anos`
                        : "—"}
                    </p>
                  </div>
                </div>
          ...
          {perfil.perfil === "setor_pessoal" && pagina === "pessoal" && (
            <section className="mx-auto max-w-7xl py-2 md:py-4">
              <div className="mb-6">
                <p className="text-sm font-semibold text-brand-400">
                ...
                <h2 className="mt-1 text-2xl font-bold">
                  Solicitações das obras
                </h2>
                <p className="mt-1 text-sm text-gray-400">
                  Admissões, desligamentos e movimentações recebidas de todas as
                  obras.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="rounded-2xl border border-brand-500/35 bg-surface p-5">
                  ...
                  <p className="mt-4 text-3xl font-bold text-brand-300">
                    {
                      solicitacoesPessoal.filter(
                ...
                <div className="rounded-2xl border border-purple-500/35 bg-surface p-5">
                  ...
                  <p className="mt-4 text-3xl font-bold text-purple-300">
                    {
                      new Set(
                        solicitacoesPessoal.map((item) => item.empresa_id),
                      ).size
                    }
                  </p>
                </div>
              ...
              <section className="mt-6 overflow-hidden rounded-2xl border border-surface-border bg-surface">
                <div className="flex flex-col gap-2 border-b border-surface-border p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h3 className="font-semibold">Fila de trabalho</h3>
                    <p className="mt-1 text-sm text-gray-400">
                      Solicitações recentes, com prioridade para as pendentes.
                    </p>
                  </div>
                  <span className="text-sm text-gray-400">
                    {solicitacoesPessoal.length} registro(s)
                  </span>
                </div>
                <div className="divide-y divide-surface-border">
                  {solicitacoesPessoal.length === 0 ? (
                        ...
                        >
                          ...
                          <div className="flex shrink-0 items-center gap-3">
                            ...
                            <span className="text-xs text-gray-500">
                              {new Date(item.solicitado_em).toLocaleDateString(
                                "pt-BR",
                              )}
                            </span>
                          </div>
          ...
          {perfil.perfil === "central" && pagina === "central" && (
            <section className="mx-auto max-w-7xl py-2 md:py-4">
              <div className="mb-6">
                <p className="text-sm font-semibold text-brand-400">
                  ESCRITÓRIO CENTRAL
                ...
                <h2 className="mt-1 text-2xl font-bold">
                  Supervisores e obras
                </h2>
                <p className="mt-1 text-sm text-gray-400">
                  Acompanhe a fila de aprovação por responsável, mantendo a
                  hierarquia do painel desktop.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-emerald-500/35 bg-surface p-5">
                  ...
                  <p className="mt-4 text-3xl font-bold text-emerald-300">
                    {resumoCentral.reduce(
                      (total, item) => total + item.obras.length,
                      0,
                    )}
                  </p>
                </div>
                <div className="rounded-2xl border border-amber-500/35 bg-surface p-5">
                  ...
                  <p className="mt-4 text-3xl font-bold text-amber-300">
                    {resumoCentral.reduce(
                      (total, item) => total + item.pendencias,
                      0,
                    )}
                  </p>
                </div>
              ...
              <section className="mt-6 overflow-hidden rounded-2xl border border-surface-border bg-surface">
                <div className="border-b border-surface-border p-5">
                  <h3 className="font-semibold">Fluxo por supervisor</h3>
                  <p className="mt-1 text-sm text-gray-400">
                    Seleção de obras e itens aguardando aprovação do Escritório.
                  </p>
                </div>
                <div className="divide-y divide-surface-border">
                  {resumoCentral.length === 0 ? (
                    <p className="p-6 text-sm text-gray-400">
                      Nenhum supervisor ativo cadastrado.
                    </p>
                  ) : (
                    resumoCentral.map((supervisor) => (
                      <article key={supervisor.id} className="p-5">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <p className="font-medium">{supervisor.nome}</p>
                            <p className="mt-1 text-sm text-gray-400">
                              {supervisor.email}
                            </p>
                          </div>
                          <span
                            className={
                              supervisor.pendencias
                                ? "w-fit rounded-full bg-amber-500/15 px-3 py-1 text-xs font-medium text-amber-300"
                                : "w-fit rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-medium text-emerald-300"
                            }
                          >
                            {supervisor.pendencias
                              ? `${supervisor.pendencias} pendência(s)`
                              : "Sem pendências"}
                          </span>
                        </div>
                        <div className="mt-4 flex flex-wrap gap-2">
                          {supervisor.obras.length === 0 ? (
                            <span className="text-sm text-gray-500">
                              Nenhuma obra vinculada.
                            </span>
                          ) : (
                            supervisor.obras.map((obra) => (
                              <span
                                key={obra.id}
                                className="rounded-lg bg-surface-hover px-3 py-1.5 text-sm text-gray-300"
                              >
                                {obra.nome}
                              </span>
                            ))
          ...
          {pagina === "inicio" && (
            <section className="mx-auto max-w-[1180px] pb-8 pt-1">
              {!resumo ? (
                <div className="rounded-xl border border-surface-border bg-surface p-6 text-sm text-gray-300">
                  Não foi possível carregar os indicadores da obra. Recarregue a
                  página ou verifique as permissões deste usuário.
                </div>
              ) : (
                <>
                  <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
                    <div>
                      ...
                      <span className="mt-1 inline-flex rounded-full bg-brand-500/15 px-2 py-0.5 text-xs text-brand-300">
                  ...
                  <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-gray-200">
                    <UsersRound size={15} className="text-brand-400" />
                    Recursos Humanos
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    <article className="min-h-[134px] rounded-xl border border-brand-500/40 bg-[#1f2d46] p-4">
                      ...
                      <p className="mt-1 text-2xl font-bold">{resumo.ativos}</p>
                      <p className="mt-1 text-xs text-gray-400">
                        Nenhum afastamento
                      </p>
                    </article>
                    <article className="min-h-[134px] rounded-xl border border-emerald-500/40 bg-[#193c34] p-4">
                      ...
                      <p className="mt-1 text-2xl font-bold text-emerald-400">
                        {resumo.custoFolha.toLocaleString("pt-BR", {
                          style: "currency",
                          currency: "BRL",
                        })}
                      </p>
                      <p className="mt-1 text-xs text-gray-400">Folha mensal</p>
                    </article>
                    <article className="min-h-[134px] rounded-xl border border-amber-500/40 bg-[#3a2e17] p-4">
                      ...
                      <p className="mt-1 text-2xl font-bold text-amber-300">
                        {resumo.mediaIdade ? `${resumo.mediaIdade} anos` : "—"}
                      </p>
                      <p className="mt-1 text-xs text-gray-400">
                        Colaboradores ativos
                      </p>
                    </article>
                    <article className="min-h-[134px] rounded-xl border border-purple-500/40 bg-[#34283d] p-4">
                      <span className="grid h-8 w-8 place-items-center rounded-lg bg-purple-500 text-white">
                      ...
                      <p className="mt-1 text-2xl font-bold text-purple-300">
                        {resumo.aniversariantes.length}
                      </p>
                      <p className="mt-1 truncate text-xs text-gray-400">
                        {resumo.aniversariantes[0]?.nome ??
                          "Nenhum aniversariante"}
                      </p>
                    </article>
                  </div>
                  <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,2.1fr)_minmax(300px,1fr)]">
                    <section className="overflow-hidden rounded-xl border border-surface-border bg-[#1f2d46]">
                      <div className="flex items-center gap-2 border-b border-surface-border px-4 py-3 text-sm font-semibold">
                        <UsersRound size={15} className="text-brand-400" />
                        Colaboradores por função
                      </div>
                      <div className="space-y-3 p-4">
                        {resumo.porFuncao.length === 0 ? (
                          <p className="text-sm text-gray-400">
                            Sem funções cadastradas.
                          </p>
                        ) : (
                          resumo.porFuncao.slice(0, 8).map((item) => (
                            <div key={item.funcao}>
                              <div className="flex justify-between gap-3 text-xs">
                                <strong className="truncate text-gray-100">
                                  {item.funcao}
                                </strong>
                              ...
                              <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[#33425f]">
                                <div
                                  className="h-full rounded-full bg-[#2f80ed]"
                                  style={{
                                    width: `${Math.max(4, (item.quantidade / Math.max(resumo.ativos, 1)) * 100)}%`,
                                  }}
                                />
                              </div>
                            </div>
                    ...
                    <section className="overflow-hidden rounded-xl border border-surface-border bg-[#1f2d46]">
                      <div className="flex items-center gap-2 border-b border-surface-border px-4 py-3 text-sm font-semibold">
                        <CalendarDays size={15} className="text-purple-300" />
                        Aniversariantes do mês
                      </div>
                      <div className="space-y-3 p-4">
                        {resumo.aniversariantes.length === 0 ? (
                          ...
                          resumo.aniversariantes.slice(0, 6).map((item) => (
                            <div
                              key={`${item.nome}-${item.nascimento}`}
                              className="flex items-center gap-3"
                            >
                              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-purple-500/20 text-xs text-purple-300">
                                {item.nascimento.slice(8, 10)}
                              </span>
                              <div className="min-w-0">
              ...
              <div className="mx-auto max-w-7xl">
                <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-1 text-2xl font-bold">Lançamentos</h2>
                    <p className="mt-1 text-sm text-gray-400">
                      Controle de receitas, despesas e compromissos da obra.
                    </p>
                  </div>
                  {perfil.perfil === "admin" && (
                    <button
                ...
                <div className="mb-5 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-xl border border-emerald-500/30 bg-surface p-4">
                    ...
                    <p className="mt-2 text-xl font-bold text-emerald-300">
                      {resumoFinanceiro.receitas.toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL",
                      })}
                    </p>
                  </div>
                  <div className="rounded-xl border border-red-500/30 bg-surface p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-red-300">
                      Despesas
                    </p>
                    <p className="mt-2 text-xl font-bold text-red-300">
                      {resumoFinanceiro.despesas.toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL",
                      })}
                    </p>
                ...
                {novoLancamento && (
                  <form
                    ...
                    className="mb-5 grid gap-3 rounded-lg border border-surface-border bg-surface p-4 md:grid-cols-2"
                  >
                    <label className="text-sm text-gray-300 md:col-span-2">
                      Descrição
                      <input
                    ...
                    <label className="text-sm text-gray-300">
                      ...
                      <input
                        className="mt-1 w-full rounded-md border border-surface-border bg-surface-card px-3 py-2 text-white"
                        type="date"
                        value={formLancamento.data_venc}
                        onChange={(e) =>
                          setFormLancamento({
                            ...formLancamento,
                            data_venc: e.target.value,
                          })
                        }
                      />
                    </label>
                ...
                <div className="mb-4 flex flex-col gap-3 sm:flex-row">
                  <input
                    className="w-full rounded-lg border border-surface-border bg-surface px-3 py-2.5 text-sm text-white sm:max-w-md"
                  ...
                  <select
                    ...
                    className="rounded-lg border border-surface-border bg-surface px-3 py-2.5 text-sm text-gray-200"
                    ...
                    <tbody>
                      {lancamentosFiltrados.length === 0 ? (
                        ...
                        lancamentosFiltrados.map((item) => (
                          ...
                          >
                            <td className="p-3 text-gray-400">{item.data}</td>
                            <td className="p-3">{item.descricao}</td>
                            <td className="p-3">
                              <span className="rounded bg-surface px-2 py-1 text-xs">
                                {item.status}
                              </span>
                            </td>
                            <td
                              className={
                                item.tipo === "receita"
                                  ? "p-3 text-right font-medium text-emerald-400"
                                  : "p-3 text-right font-medium text-red-300"
                              }
                            >
                              {item.tipo === "receita" ? "+" : "-"}{" "}
                              {Number(item.valor).toLocaleString("pt-BR", {
              ...
              <div className="mx-auto max-w-7xl">
                <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-1 text-2xl font-bold">Colaboradores</h2>
                    <p className="mt-1 text-sm text-gray-400">
                      Cadastro, situação e custo da equipe da obra.
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                <div className="mb-5 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-xl border border-emerald-500/30 bg-surface p-4">
                    ...
                    <p className="mt-2 text-2xl font-bold text-emerald-300">
                      {resumoStatusRh.ativos}
                    </p>
                  </div>
                  <div className="rounded-xl border border-brand-500/30 bg-surface p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-brand-300">
                      Em férias
                    </p>
                    <p className="mt-2 text-2xl font-bold text-brand-300">
                      {resumoStatusRh.ferias}
                    </p>
                  </div>
                  <div className="rounded-xl border border-amber-500/30 bg-surface p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-amber-300">
                ...
                <div className="mb-4 flex flex-col gap-3 sm:flex-row">
                  ...
                  <select
                    ...
                    className="rounded-lg border border-surface-border bg-surface px-3 py-2.5 text-sm text-gray-200"
                    value={filtroStatusRh}
                    onChange={(e) => setFiltroStatusRh(e.target.value)}
                  >
                  ...
                  >
                    ...
                    <label className="text-sm text-gray-300">
                      ...
                      <input
                        ...
                        step="0.01"
                        type="number"
                        value={formColaborador.salario_base}
                        onChange={(e) =>
                          setFormColaborador({
                            ...formColaborador,
                          ...
                          <span
                            className={
                              item.status === "ativo"
                                ...
                                : "rounded-full bg-gray-500/10 px-2.5 py-1 text-xs text-gray-300"
                            }
                          >
                            {item.status}
                          </span>
                        </div>
                        <div className="mt-4 grid grid-cols-2 gap-3 border-t border-surface-border pt-3 text-sm">
                          <div>
                          ...
                          <div>
                            ...
                            <p className="mt-1 font-medium text-gray-200">
                              {Number(item.salario_base).toLocaleString(
                                "pt-BR",
                          ...
                          >
                            ...
                            <td className="p-3">
                              <span
                                className={
                                  item.status === "ativo"
                                    ? "rounded bg-emerald-500/10 px-2 py-1 text-xs text-emerald-300"
                                    : "rounded bg-gray-500/10 px-2 py-1 text-xs text-gray-400"
                                }
                              >
                                {item.status}
                              </span>
          ...
          {pagina === "estoque" && (
            <section className="mx-auto mt-7 max-w-7xl">
              <div className="mb-6">
                ...
                <h2 className="mt-1 text-2xl font-bold">Estoque</h2>
                <p className="mt-1 text-sm text-gray-400">
                  Consulta rápida de produtos e alertas de reposição.
                </p>
              </div>
              <div className="mb-5 grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-brand-500/30 bg-surface p-4">
                ...
                <div className="rounded-xl border border-emerald-500/30 bg-surface p-4">
                  ...
                  <p className="mt-2 text-2xl font-bold text-emerald-300">
                    {produtosWeb.length - produtosAbaixoMinimo}
                  </p>
                </div>
              </div>
              <input
                    ...
                    >
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3 border-t border-surface-border pt-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="mt-1 font-medium text-gray-200">
                            {Number(item.valor_unitario).toLocaleString(
                              "pt-BR",
                              { style: "currency", currency: "BRL" },
                            )}
                          </p>
                    ...
                    {produtosFiltrados.length === 0 ? (
                      ...
                      produtosFiltrados.map((item) => (
                        <tr
                          ...
                          className="border-t border-surface-border"
                        >
                          <td className="p-3 text-gray-400">{item.codigo}</td>
                          <td className="p-3 font-medium">{item.nome}</td>
                          <td
                            className={
                              Number(item.estoque_atual) <=
          ...
          {pagina === "almox" && (
            <section className="mx-auto max-w-[1180px] py-2 md:py-4">
              <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-brand-400">
                    ALMOXARIFADO
                  </p>
                  <h2 className="mt-1 text-2xl font-bold">Painel inicial</h2>
                  <p className="mt-1 text-sm text-gray-400">
                    Resumo de materiais e movimentações recentes da obra.
                  </p>
                </div>
                {perfil.perfil !== "gestor" && (
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <article className="rounded-xl border border-red-500/35 bg-[#342326] p-4">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    materiais que precisam de reposição
                  </p>
                </article>
                <article className="rounded-xl border border-amber-500/35 bg-[#352c1f] p-4">
                ...
                <article className="rounded-xl border border-emerald-500/35 bg-[#1d352b] p-4">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    posição atual dos materiais
                  </p>
                </article>
              </div>
              <div className="mt-5 grid gap-4 xl:grid-cols-2">
                <section className="overflow-hidden rounded-xl border border-surface-border bg-[#1f2d46]">
                  <h3 className="border-b border-surface-border px-4 py-3 text-sm font-semibold text-emerald-300">
                    Últimas entradas
                  </h3>
                  <div className="divide-y divide-surface-border">
                    {entradasWeb.slice(0, 6).map((item) => (
                      <div
                        key={item.id}
                ...
                <section className="overflow-hidden rounded-xl border border-surface-border bg-[#1f2d46]">
                  <h3 className="border-b border-surface-border px-4 py-3 text-sm font-semibold text-amber-300">
                    Últimas saídas
                  </h3>
                  <div className="divide-y divide-surface-border">
                    {saidasWeb.slice(0, 6).map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between gap-3 px-4 py-3"
                      >
            ...
            <section className="mx-auto max-w-[1180px] py-2 md:py-4">
              <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-bold">Entradas</h2>
                  <p className="mt-1 text-sm text-gray-400">
                    Recebimentos de materiais e ferramentas.
                  </p>
                </div>
                <button
                  className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium hover:bg-emerald-500"
                  onClick={() => setNovaEntrada((aberto) => !aberto)}
                >
                  {novaEntrada ? "Cancelar" : "+ Nova entrada"}
                </button>
              </div>
              {novaEntrada && (
                <form
                  onSubmit={salvarEntrada}
                  className="mb-5 grid gap-3 rounded-xl border border-surface-border bg-surface p-4 md:grid-cols-2"
                >
                  <label className="text-sm text-gray-300">
                    Material
                    <select
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formEntrada.fornecedor_nome}
                      onChange={(e) =>
                        setFormEntrada({
                          ...formEntrada,
                  ...
                  <div className="md:col-span-2">
                    <button
                      className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium disabled:opacity-60"
                ...
                <table className="w-full min-w-[620px] text-left text-sm">
                  ...
                  <tbody>
                    {entradasWeb.length === 0 ? (
                      ...
                      entradasWeb.map((item) => (
                        <tr
                          key={item.id}
                          className="border-t border-surface-border"
                        >
                          <td className="p-3 text-gray-300">{item.data}</td>
            ...
            <section className="mx-auto max-w-[1180px] py-2 md:py-4">
              <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-bold">Saídas</h2>
                  <p className="mt-1 text-sm text-gray-400">
                    Baixas de materiais retirados da obra.
                  </p>
                </div>
                <button
                  className="rounded-lg bg-amber-600 px-4 py-2.5 text-sm font-medium hover:bg-amber-500"
                  onClick={() => setNovaSaida((aberto) => !aberto)}
                >
                  {novaSaida ? "Cancelar" : "+ Nova saída"}
                </button>
              </div>
              {novaSaida && (
                <form
                ...
                >
                  <label className="text-sm text-gray-300">
                    ...
                    >
                      ...
                      {produtosWeb.map((item) => (
                        <option key={item.id} value={item.id}>
                          {item.codigo} — {item.nome} ({item.estoque_atual}{" "}
                          {item.unidade ?? ""})
                        </option>
                      ))}
                    </select>
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formSaida.retirado_por_nome}
                      onChange={(e) =>
                        setFormSaida({
                          ...formSaida,
                          retirado_por_nome: e.target.value,
                        })
                      }
                      required
                    />
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formSaida.setor}
                      onChange={(e) =>
                        setFormSaida({ ...formSaida, setor: e.target.value })
                      }
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                  ...
                  <div className="md:col-span-2">
                    <button
                      className="rounded-lg bg-amber-600 px-4 py-2.5 text-sm font-medium disabled:opacity-60"
                      disabled={salvandoSaida}
                    >
                      {salvandoSaida ? "Salvando…" : "Registrar saída"}
                    </button>
                  </div>
                </form>
              )}
          ...
          {pagina === "notas" && (
            <section className="mx-auto max-w-[1180px] py-2 md:py-4">
              <div className="mb-6">
                ...
                <h2 className="mt-1 text-2xl font-bold">Notas fiscais</h2>
                <p className="mt-1 text-sm text-gray-400">
              ...
              <div className="rounded-xl border border-surface-border bg-surface p-4">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-400">
                      Visualize, baixe ou imprima os arquivos armazenados com
                  ...
                  <div className="flex flex-wrap gap-2">
                    <select
                      ...
                      value={documentoFinanceiroTipo}
                      onChange={(e) => {
                        const tipo = e.target.value as "ap" | "nf";
                        setDocumentoFinanceiroTipo(tipo);
                        setDocumentoFinanceiroId("");
                        setAnexosFinanceiros([]);
                      }}
                    >
                      <option value="ap">Autorização de pagamento</option>
                      <option value="nf">Nota fiscal</option>
                    </select>
                    <select
                      aria-label="Documento financeiro"
                      className="max-w-72 rounded-lg border border-surface-border bg-surface-card px-3 py-2 text-sm text-white"
                      value={documentoFinanceiroId}
                      onChange={(e) => {
                        setDocumentoFinanceiroId(e.target.value);
                    ...
                    >
                      ...
                      {(documentoFinanceiroTipo === "ap"
                        ...
                        <option key={item.id} value={item.id}>
                          {documentoFinanceiroTipo === "ap"
                            ? `${(item as AutorizacaoWeb).beneficiario_nome} — ${(item as AutorizacaoWeb).valor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}`
                            : `${(item as NotaFiscalWeb).fornecedor_nome} — NF ${(item as NotaFiscalWeb).numero_nf ?? "—"}`}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                {documentoFinanceiroId && (
                  <>
                        ...
                        anexosFinanceiros.map((anexo, indice) => (
                          ...
                          >
                            ...
                            <div className="flex shrink-0 gap-2">
                              <button
                                className="rounded-md border border-surface-border px-3 py-1.5 text-xs text-gray-200 hover:bg-surface-hover"
                                onClick={() =>
                    ...
                    {["admin", "master"].includes(perfil.perfil) && (
                      <form
                        onSubmit={enviarAnexosFinanceiros}
                        className="mt-4 border-t border-surface-border pt-4"
                      >
                        <label className="block text-sm text-gray-300">
                          Anexar documentos
                          <input
                            className="mt-2 block w-full text-sm text-gray-300 file:mr-3 file:rounded-lg file:border-0 file:bg-brand-600 file:px-3 file:py-2 file:text-sm file:font-medium file:text-white"
                            type="file"
                            multiple
                            onChange={(e) =>
                              setNovosAnexosFinanceiros(
                        ...
                        {novosAnexosFinanceiros.length > 0 && (
                          <p className="mt-2 text-xs text-gray-400">
                            {novosAnexosFinanceiros
                              .map((arquivo) => arquivo.name)
                              .join(", ")}
                          </p>
                        )}
                        <button
                          className="mt-3 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium disabled:opacity-60"
                          disabled={enviandoAnexosFinanceiros}
                        >
                          {enviandoAnexosFinanceiros
                            ? "Enviando…"
                            : "Enviar anexos"}
                        </button>
                      </form>
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <h3 className="font-semibold">
                      Documentos dos itens em análise
                    </h3>
                    <p className="mt-1 text-sm text-gray-400">
                      Abra, baixe ou imprima os anexos antes de registrar sua
                      aprovação.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    >
                      ...
                      {(documentoFinanceiroTipo === "ap"
                        ...
                        <option key={item.id} value={item.id}>
                          {documentoFinanceiroTipo === "ap"
                            ? (item as AutorizacaoWeb).beneficiario_nome
                            : `${(item as NotaFiscalWeb).fornecedor_nome} — NF ${(item as NotaFiscalWeb).numero_nf ?? "—"}`}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="mt-4 divide-y divide-surface-border rounded-lg border border-surface-border">
                    {carregandoAnexosFinanceiros ? (
                      ...
                      anexosFinanceiros.map((anexo, indice) => (
                        ...
                        >
                          <p className="min-w-0 truncate text-sm">
                            {caminhoStorage(anexo.caminho).split("/").at(-1) ??
                              `Documento ${indice + 1}`}
                          </p>
                          <div className="flex shrink-0 gap-2">
                            <button
                            ...
                            <button
                              className="rounded-md border border-surface-border px-3 py-1.5 text-xs text-gray-200 hover:bg-surface-hover"
                              onClick={() => void baixarArquivo(anexo.caminho)}
                            >
                              Baixar
                            </button>
                            <button
          ...
          {pagina === "ap" && perfil.perfil === "admin" && (
            <section className="mx-auto max-w-[1180px] pb-7">
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-surface-border bg-surface p-4">
                <div>
                  <p className="font-semibold">Nova autorização de pagamento</p>
                  <p className="mt-1 text-sm text-gray-400">
                    O lançamento financeiro é criado junto com a autorização.
                  </p>
                </div>
                <button
                  className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium"
                  onClick={() => setNovaAp((aberto) => !aberto)}
                >
                  {novaAp ? "Cancelar" : "+ Nova AP"}
                </button>
              </div>
              {novaAp && (
                <form
                  onSubmit={salvarAp}
                  className="mt-4 grid gap-3 rounded-xl border border-surface-border bg-surface p-4 md:grid-cols-2"
                >
                  <label className="text-sm text-gray-300">
                    Fornecedor
                    <select
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formAp.fornecedor_id}
                      onChange={(e) =>
                        setFormAp({ ...formAp, fornecedor_id: e.target.value })
                      }
                      required
                    >
                      <option value="">Selecione</option>
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      type="date"
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formAp.vencimento}
                      onChange={(e) =>
                        setFormAp({ ...formAp, vencimento: e.target.value })
                      }
                      required
                    />
                  </label>
                  <label className="text-sm text-gray-300 md:col-span-2">
                    ...
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formAp.descricao}
                      onChange={(e) =>
                        setFormAp({ ...formAp, descricao: e.target.value })
                      }
                      required
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      ...
                      value={formAp.valor}
                      onChange={(e) =>
                        setFormAp({ ...formAp, valor: e.target.value })
                      }
                      required
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formAp.observacoes}
                      onChange={(e) =>
                        setFormAp({ ...formAp, observacoes: e.target.value })
                      }
                    />
                  </label>
                  <div className="md:col-span-2">
                    <button
                      ...
                      disabled={salvandoAp}
                    >
                      {salvandoAp ? "Criando…" : "Criar autorização"}
                    </button>
                  </div>
                </form>
              )}
          ...
          {pagina === "notas" && perfil.perfil === "admin" && (
            <section className="mx-auto max-w-[1180px] pb-7">
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-surface-border bg-surface p-4">
                <div>
                  <p className="font-semibold">Nova nota fiscal</p>
                  <p className="mt-1 text-sm text-gray-400">
                    O boleto e o lançamento financeiro são criados juntos.
                  </p>
                </div>
                <button
                  className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium"
                  onClick={() => setNovaNf((aberto) => !aberto)}
                >
                  {novaNf ? "Cancelar" : "+ Nova nota"}
                </button>
              </div>
              {novaNf && (
                <form
                ...
                >
                  <label className="text-sm text-gray-300">
                    ...
                    <select
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formNf.fornecedor_id}
                      onChange={(e) =>
                        setFormNf({ ...formNf, fornecedor_id: e.target.value })
                      }
                      required
                    >
                      <option value="">Selecione</option>
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormNf({ ...formNf, numero_nf: e.target.value })
                      }
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formNf.numero_pedido}
                      onChange={(e) =>
                        setFormNf({ ...formNf, numero_pedido: e.target.value })
                      }
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      ...
                      value={formNf.data}
                      onChange={(e) =>
                        setFormNf({ ...formNf, data: e.target.value })
                      }
                      required
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      ...
                      value={formNf.valor}
                      onChange={(e) =>
                        setFormNf({ ...formNf, valor: e.target.value })
                      }
                      required
                    />
                  </label>
                  <label className="text-sm text-gray-300">
                    ...
                    <input
                      type="date"
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formNf.vencimento}
                      onChange={(e) =>
                        setFormNf({ ...formNf, vencimento: e.target.value })
                      }
                      required
                    />
                  </label>
                  <div className="md:col-span-2">
                    <button
                      ...
                      disabled={salvandoNf}
                    >
                      {salvandoNf ? "Criando…" : "Criar nota fiscal"}
                    </button>
                  </div>
                </form>
              )}
            ...
            <section className="mx-auto max-w-[1180px] py-2 md:py-4">
              <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-bold">Fornecedores</h2>
                  <p className="mt-1 text-sm text-gray-400">
                    Cadastros usados nas autorizações, notas fiscais e entradas.
                  </p>
                </div>
                <button
                  className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium"
                  onClick={() => setNovoFornecedor((aberto) => !aberto)}
                >
                  {novoFornecedor ? "Cancelar" : "+ Novo fornecedor"}
                </button>
              </div>
              {novoFornecedor && (
                <form
                ...
                >
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <select
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formFornecedor.tipo_pessoa}
                      onChange={(e) =>
                        setFormFornecedor({
                          ...formFornecedor,
                  ...
                  <label className="text-sm text-gray-300">
                    {formFornecedor.tipo_pessoa === "pj" ? "CNPJ" : "CPF"}
                    <input
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={
                        formFornecedor.tipo_pessoa === "pj"
                      ...
                      onChange={(e) =>
                        setFormFornecedor(
                          formFornecedor.tipo_pessoa === "pj"
                            ? { ...formFornecedor, cnpj: e.target.value }
                            : { ...formFornecedor, cpf: e.target.value },
                        )
                      }
                    />
                  </label>
                  ...
                  <label className="text-sm text-gray-300">
                    ...
                    <select
                      className="mt-1 w-full rounded-lg border border-surface-border bg-surface-card px-3 py-2.5 text-white"
                      value={formFornecedor.forma_pagamento}
                      onChange={(e) =>
                        setFormFornecedor({
                          ...formFornecedor,
                    ...
                    >
                      ...
                      <option value="conta">Conta / PIX</option>
                    </select>
                  </label>
                  <div className="md:col-span-2">
                    <button
          ...
          {pagina === "lotes" && (
            <section className="mx-auto max-w-[1180px] py-2 md:py-4">
              <div className="mb-6">
                ...
                <h2 className="mt-1 text-2xl font-bold">Lotes financeiros</h2>
                <p className="mt-1 text-sm text-gray-400">
                  Agrupe itens autorizados e envie-os para a aprovação do
                  Supervisor.
                </p>
              </div>
              <div className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(360px,1fr)]">
                <section className="rounded-xl border border-surface-border bg-surface p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <h3 className="font-semibold">Itens disponíveis</h3>
                      <p className="mt-1 text-sm text-gray-400">
                        Apenas APs e NFs autorizadas que ainda não estão em
                        lote.
                      </p>
                    </div>
                    <button
                      className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium disabled:opacity-60"
                      disabled={
                        processandoLote || itensLoteSelecionados.size === 0
                      }
                      onClick={() => void criarLoteFinanceiro()}
                    >
                      {processandoLote
                        ? "Criando…"
                        : `Criar lote (${itensLoteSelecionados.size})`}
                    </button>
                  </div>
                  <div className="mt-4 divide-y divide-surface-border rounded-lg border border-surface-border">
                    {[
                      ...autorizacoesWeb
                        .filter(
                          (item) =>
                            !!item.aprovado_por && item.lote_id === null,
                        )
                        .map((item) => ({
                          chave: `ap-${item.id}`,
                          tipo: "AP",
                          nome: item.beneficiario_nome,
                          detalhe: item.descricao ?? "Autorização de pagamento",
                          valor: item.valor,
                        })),
                      ...notasFiscaisWeb
                        .filter(
                        ...
                        .map((item) => ({
                          ...
                          tipo: "NF",
                          nome: item.fornecedor_nome,
                          detalhe: `NF ${item.numero_nf ?? "—"}`,
                          valor: item.valor_total,
                        })),
                    ].map((item) => (
                      <label
                        key={item.chave}
                        className="flex cursor-pointer items-center gap-3 p-3 hover:bg-surface-hover"
                      >
                        <input
                          type="checkbox"
                          className="h-4 w-4 accent-blue-600"
                          checked={itensLoteSelecionados.has(item.chave)}
                          onChange={() => alternarItemLote(item.chave)}
                        />
                        <span className="min-w-0 flex-1">
                          <span className="mr-2 rounded bg-brand-500/15 px-1.5 py-0.5 text-[10px] font-bold text-brand-300">
                            {item.tipo}
                          </span>
                    ...
                    {autorizacoesWeb.filter(
                      (item) => !!item.aprovado_por && item.lote_id === null,
                    ).length +
                      notasFiscaisWeb.filter(
                        (item) => !!item.aprovado_por && item.lote_id === null,
                      ).length ===
                      0 && (
                      <p className="p-5 text-sm text-gray-400">
                        Não há itens autorizados disponíveis para lote.
                      </p>
                    )}
                  </div>
                </section>
                <section className="rounded-xl border border-surface-border bg-surface p-4">
                  <h3 className="font-semibold">Lotes criados</h3>
                  <p className="mt-1 text-sm text-gray-400">
                    Envie os lotes abertos ao Supervisor.
                  </p>
                  <div className="mt-4 divide-y divide-surface-border">
                    {lotesWeb.length === 0 ? (
                      <p className="py-5 text-sm text-gray-400">
                      ...
                      lotesWeb.map((lote) => (
                        <article key={lote.id} className="py-4">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <p className="font-medium">{lote.titulo}</p>
                              <p className="mt-1 text-xs text-gray-400">
                                {lote.enviado_em
                                  ? `Enviado em ${lote.enviado_em.slice(0, 10)}`
                                  : "Aguardando envio"}{" "}
                              </p>
                            </div>
                            {lote.enviado_em ? (
                              ...
                              <button
                                ...
                                onClick={() =>
                                  void enviarLoteFinanceiro(lote.id)
                                }
                              >
                                {processandoLote
                                  ? "Enviando…"
          ...
          {pagina === "ap" && perfil.perfil === "admin" && (
            <section className="mx-auto max-w-[1180px] pb-7">
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-surface-border bg-surface p-4">
                ...
                <select
                  className="min-w-60 rounded-lg border border-surface-border bg-surface-card px-3 py-2 text-sm text-white"
                  defaultValue=""
                  onChange={(e) => {
                    const item = autorizacoesWeb.find(
                      (ap) => ap.id === Number(e.target.value),
                    );
                    if (!item) return;
                    setEditandoApId(item.id);
                    setFormAp({
                      fornecedor_id: String(item.beneficiario_id),
                ...
                >
                  <option value="">Selecione uma AP</option>
                  {autorizacoesWeb
                    .filter((item) => item.lote_id === null)
                    .map((item) => (
                      <option key={item.id} value={item.id}>
                        {item.beneficiario_nome} —{" "}
                        {Number(item.valor).toLocaleString("pt-BR", {
            ...
            <section className="mx-auto max-w-[1180px] pb-7">
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-surface-border bg-surface p-4">
                ...
                <select
                  ...
                  defaultValue=""
                  onChange={(e) => {
                    const item = notasFiscaisWeb.find(
                      (nota) => nota.id === Number(e.target.value),
                    );
                    if (!item) return;
                    setEditandoNfId(item.id);
                    setFormNf({
                      fornecedor_id: item.fornecedor_id
                        ? String(item.fornecedor_id)
                        : "",
                      numero_nf: item.numero_nf ?? "",
                      numero_pedido: "",
                ...
                >
                  ...
                  {notasFiscaisWeb
                    ...
                    .map((item) => (
                      <option key={item.id} value={item.id}>
                        {item.fornecedor_nome} — NF {item.numero_nf ?? "—"}
                      </option>
                    ))}
                </select>
              </div>
          ...
          {pagina === "fornecedores" && (
            <section className="mx-auto max-w-[1180px] pb-7">
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-surface-border bg-surface p-4">
                <span className="text-sm text-gray-300">Editar fornecedor</span>
                <select
                  className="min-w-60 rounded-lg border border-surface-border bg-surface-card px-3 py-2 text-sm text-white"
                  defaultValue=""
                  onChange={(e) => {
                    const item = fornecedoresWeb.find(
                      (fornecedor) => fornecedor.id === Number(e.target.value),
                    );
                    if (!item) return;
                    setEditandoFornecedorId(item.id);
                    setFormFornecedor({
                      nome: item.nome,
          ...
          {perfil.perfil === "supervisor" &&
            ...
            anexosFinanceiros.length > 0 && (
              <section className="mx-auto max-w-7xl pb-7">
                <div className="rounded-xl border border-brand-500/35 bg-surface p-4">
                  <h3 className="font-semibold">Aplicar assinatura ao PDF</h3>
                  <p className="mt-1 text-sm text-gray-400">
                    A assinatura é gravada em uma cópia do PDF no Storage; o
                    arquivo original permanece preservado.
                  </p>
                  <div className="mt-4 space-y-2">
                    {anexosFinanceiros.map((anexo) => (
                      <div
                        key={anexo.id}
                        className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-surface-border bg-surface-card p-3"
                      >
