grant select on public.almoxarifado_entradas, public.almoxarifado_entradas_itens to authenticated;
create policy "entradas_ler_obras_autorizadas" on public.almoxarifado_entradas for select to authenticated using (public.pode_acessar_empresa(empresa_id));
create policy "itens_entrada_ler_obras_autorizadas" on public.almoxarifado_entradas_itens for select to authenticated using (exists (select 1 from public.almoxarifado_entradas e where e.id=entrada_id and public.pode_acessar_empresa(e.empresa_id)));
...
begin
  ...
  insert into public.almoxarifado_entradas (empresa_id,numero_nota,numero_pedido,data,fornecedor_id,fornecedor_nome,valor_desconto,valor_total)
  values ((p->>'empresa_id')::bigint,nullif(p->>'numero_nota',''),nullif(p->>'numero_pedido',''),p->>'data',nullif(p->>'fornecedor_id','')::bigint,p->>'fornecedor_nome',coalesce((p->>'valor_desconto')::numeric,0),total) returning id into entrada_id;
...
revoke all on function public.criar_entrada_almoxarifado(jsonb) from public;
grant execute on function public.criar_entrada_almoxarifado(jsonb) to authenticated;
